...
def sdf_summary(model, model_input, ground_truth, info, writer, iter, prefix=""):
    ...
    ## Occupancy Max-Min ##
    if writer == "wandb":
        wandb.log({prefix + "out_min": pred_sdf.min(), prefix + "out_max": pred_sdf.max()}, step=iter)
        wandb.log({prefix + "target_out_min": gt_sdf.min(), prefix + "target_out_max": gt_sdf.max()}, step=iter)
    else:
...
def point_cloud(writer, iter, name, points_xyz, colors=None):
    ...
    if writer == "wandb":
        wandb.log({name: wandb.Object3D.from_numpy(np.concatenate([points_xyz[0], colors[0]], axis=-1))}, step=iter)
