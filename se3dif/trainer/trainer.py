...
def train(model, args, train_dataloader, epochs, lr, steps_til_summary, epochs_til_checkpoint, model_dir, loss_fn,
    ...
    if rank == 0:
        ...
        if logger == 'wandb':
            wandb.init(project="analogical_grasping")
        ...
        for epoch in range(epochs):
            ...
            for step, (model_input, gt) in enumerate(train_dataloader):
                ...
                for loss_name, loss in losses.items():
                    ...
                    if rank == 0:
                        if logger == 'wandb':
                            wandb.log({"train/" + loss_name: single_loss}, step=total_steps)
                        elif logger == 'tensorboard':
                            writer.add_scalar("train/" + loss_name, single_loss, total_steps)
                    train_loss += single_loss

                train_losses.append(train_loss.item())
                if rank == 0:
                    if logger == 'wandb':
                        wandb.log({"train/loss/total": train_loss}, step=total_steps)
                    elif logger == 'tensorboard':
                        writer.add_scalar("train/loss/total", train_loss, total_steps)

                if not total_steps % steps_til_summary and rank == 0:
                    torch.save(model.state_dict(),
                               os.path.join(checkpoints_dir, 'model_current.pth'))
                    if summary_fn is not None:
                        summary_fn(model, model_input, gt, iter_info, writer, total_steps, "train/")
                ...
                if not total_steps % steps_til_summary and rank == 0:
                    ...
                    if val_dataloader is not None:
                        ...
                        for loss_name, loss in val_losses.items():
                            ...
                            if summary_fn is not None:
                                summary_fn(model, model_input, gt, val_iter_info, writer, total_steps, 'val/')
                                if logger == 'wandb':
                                    wandb.log({"val/" + loss_name: single_loss}, step=total_steps)
                                elif logger == 'tensorboard':
